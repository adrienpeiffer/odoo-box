...
Vagrant::Config.run do |config|
	...
	config.vm.provision :shell do |shell|
	  shell.inline = "echo $1 >> clone.sh"
	  shell.args = %q{"if [ ! -d ./odoo ]; then sudo -u vagrant git clone https://github.com/adrienpeiffer/odoo-box.git odoo;else cd ./odoo;git reset --hard origin/master;git pull; cd ..;fi"}
	end
